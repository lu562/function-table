...
int main(int argc, char** argv) {
	...
	e_role role;
	uint32_t bitlen = 64, numbers = 128, secparam = 128, nthreads = 1;
	uint16_t port = 7766;
	...
	// std::vector<uint64_t> input_value(1000, ((uint64_t)1 << 62) + 1);
	std::vector<uint64_t> input_value;
	for (uint64_t i = 0; i < 10000; i++) {
		input_value.push_back(5000 - i);
	}

	test_inner_product_circuit(role, address, port, seclvl, numbers, bitlen, nthreads, mt_alg, S_ARITH, input_value);
	return 0;
}
