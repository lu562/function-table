...
for i in range(k-1):
	...
	for j in range(2^l):
		...
		res = UNK
		minstr = Unk0 + inp + "0"*(n-len(Unk0+inp))
		#print("\n")
		#print(Unk0)
		#print(inp)
		#print(minstr)
		...
		#	sofsoft[b][i][j] += res
		#	continue
		maxstr = Unk0 + inp + "1"*(n-len(Unk0+inp))
		#maxv = min(int(maxstr, 2), p-1)
		maxv = int(maxstr, 2)
		...
		#print("i: " + str(i) + "  j: " + str(j) + "  inp: " + str(inp))
		#print("minstr: " + str(minstr) + "  " + "maxstr: " + str(maxstr))
	...
	for j in range(2^l):
		...
		#Function to determine results
		res = UNK
		minstr = Unk1 + inp + "0"*(n-len(Unk1+inp))
		minv = int(minstr, 2)
		#if minv >= p:
		#	res = (int)(Integers(3).random_element())
		#	res = 0
		#	sofsoft[1-b][i][j] += res
		#	continue
		maxstr = Unk1 + inp + "1"*(n-len(Unk1+inp))
		#maxv = min(int(maxstr, 2), p-1)
		maxv = int(maxstr, 2)
...
for i in range(k-1):
	for j in range(2^l):
		a = F3.random_element()
		sofsoft_1[0][i][j] = a
		sofsoft_2[0][i][j] = F3(int(sofsoft[0][i][j])) - a
		a = F3.random_element()
		sofsoft_1[1][i][j] = a
		sofsoft_2[1][i][j] = F3(int(sofsoft[1][i][j])) - a
...
for c in range(len(sofsoft)):
	for co in range(len(sofsoft[c])):
		for cou in range(len(sofsoft[c][co])):
			f.write(str(c) + "," + str(co+1) + "," + str(cou) + "," + str(sofsoft[c][co][cou]) + "\n")
f.close()
